 ...
 * An evjs plugin.
 *
 * Plugins modify the framework configuration via the `config` hook,
 * or the underlying bundler configuration via the `bundler` hook.
 *
 * @example
 * ```ts
 * function myPlugin(): EvPlugin {
 *   return {
 *     name: "evjs-plugin-example",
 *     config(config, ctx) {
 ...
 *       }
 *     },
 *     // Modify the underlying bundler directly (e.g. Webpack)
...
export interface EvPlugin {
   ...
   *
   * Called after the config file is loaded but before the build starts.
   * Can either mutate the config object directly or return a new one.
   */
  config?:
   ...
   *
   * This is called after the default bundler config is generated by the active adapter.
   * Mutate the bundler-specific config object directly (e.g., webpack configuration).
   */
 ...
 *
 * This ensures that adapters (Webpack, utoopack) can rely on
 * existence of key framework settings.
 */
